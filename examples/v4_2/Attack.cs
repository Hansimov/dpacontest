 ...
 *
 * Template for DPA contest v4.2 AES-128 Improved RSM implementation
 */
...
class Attack
{
  ...
  byte[] offsets; // The offsets used for the current trace if option --provide_offsets_v4_2 is given to the wrapper
  byte[] shuffle0; // The Shuffle0 permutation used for the current trace if option --provide_offsets_v4_2 is given to the wrapper
  byte[] shuffle10; // The Shuffle10 permutation used for the current trace if option --provide_offsets_v4_2 is given to the wrapper
  float[] samples; // The samples of the trace
  ...
  public Attack()
  {
    ...
    offsets = new byte[16];
    shuffle0 = new byte[16];
    shuffle10 = new byte[16];
    samples = new float[1704402];
  ...
  public void LaunchAttack()
  {
	...
	Read(shuffle0, shuffle0.Length);
	Read(shuffle10, shuffle10.Length);
	Read(offsets, offsets.Length);

	// Convert the samples to signed bytes
	for (uint index = 0; index < samples.Length; index++)
	  {
	    samples[index] = ReadFloat();
	  }
	...
	// - shuffle10: the Shuffle10 (0 unless the wrapper is launched with --provide_offsets_v4_2)
