...
for iteration = 1:num_traces
    ...
    samples = [samples(4798:4827) samples(9140:9169) samples(13469:13498) samples(17823:17852) samples(22163:22192) samples(26506:26535) samples(30846:30875) samples(35189:35218) ...
    ...
    %Mask power 4979,9139,13468,17822,22162,26505,30845,35188,39530,43872,48211,52554,56895,61238,65578,69920
    %Sbox power
    %228880,319696,297990,277478,256953,233990,326559,306048,285522,251643,240850,334615,314092,291125,269419,248907
    ...
    for i=1:16
        for guesskey=0:255
            var_intermedia = s2intermedia(i,guesskey+1)/num_traces;
            if (var_intermedia == 0)
                r(i,guesskey+1) = 0;
            else
                for j=1:468
                    var_samples = (s2samples(j) - ssamples(j) * ssamples(j) / num_traces)/num_traces + (s2samples(j+12) - ssamples(j+12) * ssamples(j+12) / num_traces)/num_traces;
                    var_samples = var_samples / 2;
                    if (var_samples == 0)
                        tmpr(j) = 0;
                    else
                        tmpr(j) = (sd(i,guesskey+1,j) - sd(i,guesskey+1,j+12) + ssamples(j+12) * sintermedia(i,guesskey+1)/num_traces - ssamples(j) * sintermedia(i,guesskey+1)/num_traces)/num_traces / sqrt(var_intermedia) / sqrt(var_samples);
                        tmpr(j) = tmpr(j) /2;
                    end
                end
