...
for iteration = 1:num_traces
    ...
    for i=1:16
        for guesskey=0:255
            var_intermedia = (s2intermedia(i,guesskey+1) - sintermedia(i,guesskey+1)*sintermedia(i,guesskey+1)/iteration) ...
                            / iteration;
            if (var_intermedia == 0)
                r(i,guesskey+1) = 0;
            else
                for j=1:30
                    var_samples = (s2samples(j+(i-1)*30) - ssamples(j+(i-1)*30) * ssamples(j+(i-1)*30) / iteration) ...
                                / iteration;
                    if (var_samples == 0)
                        tmpr(j) = 0;
                    else
                        tmpr(j) = (sd(i,guesskey+1,j) - ssamples(j+(i-1)*30) * sintermedia(i,guesskey+1)/iteration) ...
